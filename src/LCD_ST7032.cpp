...
void LCD_ST7032::set_cursor(uint8_t line, uint8_t pos) 
{
	uint8_t p;
	if(pos > 15) pos = 0;
...
void LCD_ST7032::no_display() //turn off display
{ 
	display_on_off_setting &= ~DISPLAY_ON_OFF_D;
	write_instruction(display_on_off_setting);
}
...
void LCD_ST7032::no_cursor() //stop display underline cursor
{ 
	display_on_off_setting &= ~DISPLAY_ON_OFF_C;
	write_instruction(display_on_off_setting);
}
...
void LCD_ST7032::no_blink() //stop cursor block blink
{ 
	display_on_off_setting &= ~DISPLAY_ON_OFF_B;
	write_instruction(display_on_off_setting);
}

void LCD_ST7032::set_contrast(int val) 
{
...
void LCD_ST7032::adjust_contrast(int val) 
{
	set_contrast(val + contrast);
}

uint8_t LCD_ST7032::get_contrast() 
{
	return contrast;
}
